# Writer Built-In Skill Library Manifest

This manifest records the Milestone 3.5 disposition for every source `SKILL.md` under `docs/skills/important` and `docs/skills/suggested/fiction`. It is the planning surface for the Writer-native rewrites; it is not the final installable library yet.

Invocation syntax reminder for future workers: `$` is a skill mention, `/` is a command, and `@` is an entity mention.

## Default Skills

| Writer skill/source skill | Source path(s) | Status | Category | Script status | Asset/source dependency notes | Rewrite notes |
| --- | --- | --- | --- | --- | --- | --- |
...
| `$story-collaborator` | `docs/skills/suggested/fiction/core/story-collaborator` | Default built-in | Core co-writing | No scripts | Skill text only | Rewrite as active co-writing partner with Writer guardrails. |
...
| `$worldbuilding-brainstorm` | `docs/skills/important/worldbuilding-brainstorm` | Default built-in | Worldbuilding | No scripts | `WORLD-FILE-FORMAT.md`, agent config | Non-negotiable default. Rewrite as canon-safe lore brainstorming inside Writer. |
...
| `$prose-polish` | `docs/skills/suggested/fiction/craft/prose-style` | Default built-in | Craft/prose | Optional helper scripts | Optional `scripts/` rhythm and prose checks | Rewrite as Writer polish/editing aid, not a detached prose-style prompt. |
...
| `$multi-pov` | `docs/skills/suggested/fiction/structure/perspectival-constellation` | Optional built-in | Structure/POV | No scripts | Skill text only | Rename for clear Writer-facing POV guidance. |
...
| `$underdog-team` | `docs/skills/suggested/fiction/character/underdog-unit` | Optional built-in | Character/team dynamics | No scripts | Skill text only | Rename for clearer Writer-facing team-story use. |
...
| `$children-stories` | No source folder yet | Optional built-in (Writer-native) | Application/form | No scripts yet | New Writer-native skill; source TBD | Reserve slot now so later workers can author the rewrite in place. |
...
| `None - deferred` | `docs/skills/suggested/fiction/structure/reverse-outliner` | Archived/deferred | Structure/analysis | Deferred; script pipeline dependent | `data/` plus multiple `scripts/` | No final 3.5 built-in `$` name. Defer until a Writer-native analysis pipeline can replace the current file/script workflow. |
...
| `None - archived` | `docs/skills/suggested/fiction/application/game-facilitator` | Archived/deferred | GM/runtime play | Deferred; script-heavy | Session/NPC/complication scripts | No final 3.5 built-in `$` name. Out of Writer core scope; archive. |
...
| `None - archived` | `docs/skills/suggested/fiction/core/story-idea-generator` | Archived/deferred | Core ideation | No scripts | Skill text only | No final 3.5 built-in `$` name. Reviewed intentionally but not shipping in 3.5; overlaps default coaching/collaboration/drafting set and needs Writer-native reframing. |
...
- Every source `SKILL.md` is represented here as one of:
  ...
  - a merged source feeding a rewritten Writer skill,
