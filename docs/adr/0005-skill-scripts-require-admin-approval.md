...
Approved scripts run through the Skill Script Runtime. Open Edda supplies service-prepared JSON inputs and an empty temporary working directory, then accepts only structured JSON outputs: reports, proposals, generated data, or drafts. Inputs may be assembled from indexed project files, saved content hashes, skill assets, and operational database records. Scripts must not directly mutate Story Text, Story Bible Entries, Entry Sections, Project Notes, Attached Notes, project files, `.edda/` metadata, or project structure.
